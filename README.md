...
## 🍴 Fork 修复说明
这个 fork 在上游版本基础上补充了几处 PPT 创建流程相关修复：

- Codex 图片生成流在返回空结果或仅返回 partial image 时，会保留可用图片并对瞬时空结果触发重试。
- 单页图片生成任务失败但页面已有生成图时，不再直接把页面状态覆盖为失败。
- 首页从想法、大纲、页面描述创建 PPT 时，支持只上传已解析完成的参考文件，不必额外手动输入文字。
- 修正 PPT 翻新页面重新生成时的服务初始化参数。
