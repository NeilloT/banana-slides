...
Usually an OCR API configuration issue. Make sure the selected OCR provider is configured correctly:

- Baidu: check `BAIDU_API_KEY`
- Azure: check `AZURE_DOCUMENT_INTELLIGENCE_ENDPOINT` and `AZURE_DOCUMENT_INTELLIGENCE_KEY`

See [Configuration](/configuration#ocr-provider).
